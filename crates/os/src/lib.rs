...
where PCS: PerContractStorage + 'static
{
    ...
    let os_program = Program::from_bytes(complied_os, Some(cairo_run_config.entrypoint)).map_err(|e| ArcaneError::Runner(e.into()))?;
    ...
    let end = cairo_runner.initialize(&mut virtual_machine, allow_missing_builtin).map_err(|e| ArcaneError::Runner(e.into()))?;
    ...
    cairo_runner.run_until_pc(end, &mut virtual_machine, &mut sn_hint_processor)
        .map_err(|e| VmException::from_vm_error(&cairo_runner, &virtual_machine, e))
        .map_err(|err| ArcaneError::Runner(err.into()))?;

    cairo_runner.end_run(cairo_run_config.disable_trace_padding, false, &mut virtual_machine, &mut sn_hint_processor)
        .map_err(|e| ArcaneError::Runner(e.into()))?;
