...
use crate::starknet::core::os::contract_class::compiled_class_hash_object::{
...
pub fn write_class(
    ...
    load_casm_entrypoints(
        vm,
        (class_base + 1)?,
        &cairo_lang_class.entry_points_by_type.external,
    )?;
    load_casm_entrypoints(
        vm,
        (class_base + 3)?,
        &cairo_lang_class.entry_points_by_type.l1_handler,
    )?;
    load_casm_entrypoints(
        vm,
        (class_base + 5)?,
        &cairo_lang_class.entry_points_by_type.constructor,
    )?;

    let bytecode: Vec<_> = cairo_lang_class
        .bytecode
        .iter()
        .map(|x| x.value.clone())
        .collect();

    let bytecode_segment_lengths = cairo_lang_class
        .bytecode_segment_lengths
        .unwrap_or(NestedIntList::Leaf(bytecode.len()));

    let bytecode_segment_structure =
        create_bytecode_segment_structure(&bytecode, bytecode_segment_lengths, visited_pcs)?;
    let bytecode_with_skipped_segments =
        bytecode_segment_structure.bytecode_with_skipped_segments();

    let data: Vec<MaybeRelocatable> = bytecode_with_skipped_segments
        .into_iter()
...
pub fn create_bytecode_segment_structure(
    ...
    let (res, total_len) = _create_bytecode_segment_structure_inner(
        bytecode,
        bytecode_segment_lengths,
        &mut rev_visited_pcs,
        0,
    ...
    if total_len != bytecode.len() {
        return Err(HintError::AssertionFailed(
            format!(
                "Invalid length bytecode segment structure: {total_len}. Bytecode length: {}.",
                bytecode.len()
            )
            .into_boxed_str(),
        ));
    ...
    if !rev_visited_pcs.is_empty() {
        return Err(HintError::AssertionFailed(
            format!(
                "PC {} is out of range.",
                rev_visited_pcs[rev_visited_pcs.len() - 1]
            )
            .into_boxed_str(),
        ));
    }
    ...
    match bytecode_segment_lengths {
        NestedIntList::Leaf(length) => {
            ...
            while !visited_pcs.is_empty()
                && (bytecode_offset..segment_end)
                    .contains(&(visited_pcs[visited_pcs.len() - 1].to_usize().unwrap()))
            {
                visited_pcs.pop();
            }
        ...
        NestedIntList::Node(lengths) => {
            ...
            for item in lengths {
                ...
                let (current_structure, item_len) = _create_bytecode_segment_structure_inner(
                    bytecode,
                    item,
                    visited_pcs,
                    bytecode_offset,
                ...
                if is_used {
                    let visited_pc_before =
                        visited_pc_before.expect("if is_used, visited_pc_before should be set");
...
pub fn get_deprecated_contract_class_struct(
    ...
    let contract_definition_dump =
        serde_json::to_vec(&deprecated_class).expect("Serialization should not fail");
    ...
    prepare_json_contract_definition(&mut cairo_contract_class_json).map_err(|_| {
        custom_hint_error("Processing Cairo contracts for backward compatibility failed")
    })?;

    let hinted_class_hash = {
        let class_hash = compute_cairo_hinted_class_hash(&cairo_contract_class_json)
            .expect("Hashing should not fail here");
    ...
    let data: Vec<MaybeRelocatable> = data
        .into_iter()
        .map(|datum| MaybeRelocatable::from(Felt252::from_hex(&datum).unwrap()))
        .collect();
