...
pub fn felt_vm2usize(felt_op: Option<&Felt252>) -> Result<usize, ArcaneError> {
    match felt_op {
        Some(felt) => {
            let big_num: u16 = felt.to_bigint().try_into().map_err(|e| ArcaneError::Output(format!("{e}")))?;

            Ok(big_num.into())
        }
        None => Err(ArcaneError::CatchAll("no length available".to_string())),
    }
}
