    ...
    let bytecode_segments = match bytecode_segment_structure {
        ...
        BytecodeSegmentStructureImpl::Leaf(_) => {
            return Err(HintError::AssertionFailed(
                "Expected SegmentedNode".to_string().into_boxed_str(),
            ));
        }
    };
...
pub fn assert_end_of_bytecode_segments(
    ...
    let bytecode_segments = exec_scopes
        .get_mut_ref::<<Vec<BytecodeSegment> as IntoIterator>::IntoIter>(
    ...
    if bytecode_segments.next().is_some() {
        return Err(HintError::AssertionFailed(
            "bytecode_segments is not exhausted"
                .to_string()
                .into_boxed_str(),
        ));
    }
...
pub fn iter_current_segment_info(
    ...
    let is_leaf = matches!(
        current_segment_info.inner_structure,
        BytecodeSegmentStructureImpl::Leaf(_)
    );
    let is_used_leaf = is_used && is_leaf;
    let is_used_leaf_felt = if is_used_leaf {
        Felt252::ONE
    } else {
        Felt252::ZERO
    };
    insert_value_from_var_name(
        vars::ids::IS_USED_LEAF,
        is_used_leaf_felt,
        vm,
        ids_data,
        ap_tracking,
...
pub fn set_ap_to_segment_hash(
    ...
    // Calc hash
    let hash = bytecode_segment_structure
        .hash()
        .map_err(|err| custom_hint_error(err.to_string()))?;
