...
use crate::r#type::deprecated_compiled_class::GenericDeprecatedCompiledClass;
...
pub fn load_deprecated_class(
    ...
    let computed_hash_addr =
        get_ptr_from_var_name(vars::ids::COMPILED_CLASS_FACT, vm, ids_data, ap_tracking)?;
    let computed_hash = vm.get_integer(computed_hash_addr)?;
    let expected_hash = exec_scopes
        .get::<Felt252>(vars::scopes::COMPILED_CLASS_HASH)
        .unwrap();
    ...
    let dep_class =
        exec_scopes.get::<GenericDeprecatedCompiledClass>(vars::scopes::COMPILED_CLASS)?;
    let dep_class = dep_class
...
pub fn load_deprecated_class_facts(
    ...
    exec_scopes.insert_value(
        vars::scopes::DEPRECATED_CLASS_HASHES,
...
pub fn load_deprecated_class_inner(
    ...
    let deprecated_class_iter = exec_scopes
        .get_mut_ref::<IntoIter<Felt252, GenericDeprecatedCompiledClass>>(
            vars::scopes::COMPILED_CLASS_FACTS,
        )?;
    ...
    let starknet_api_class = deprecated_class
        .to_starknet_api_contract_class()
        .map_err(|e| custom_hint_error(e.to_string()))?;
