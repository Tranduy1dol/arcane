    ...
    PCS: PerContractStorage + 'static,
{
    ...
    let execution_context_ptr = get_relocatable_from_var_name(
        vars::ids::VALIDATE_DECLARE_EXECUTION_CONTEXT,
        vm,
        ids_data,
        ap_tracking,
    )?;
    let deprecated_tx_info_ptr =
        (execution_context_ptr + ExecutionContext::deprecated_tx_info_offset())?;
...
where
    PCS: PerContractStorage + 'static,
{
    execute_coroutine(start_tx_validate_declare_execution_context_async::<PCS>(
        vm,
        exec_scopes,
        ids_data,
        ap_tracking,
...
pub const SET_SHA256_SEGMENT_IN_SYSCALL_HANDLER: &str =
...
where
    PCS: PerContractStorage + 'static,
{
    execute_coroutine(set_sha256_segment_in_syscall_handler_async::<PCS>(
        vm,
        exec_scopes,
        ids_data,
        ap_tracking,
...
pub fn set_component_hashes(
    ...
    let tx_class_hash = tx.class_hash.ok_or(HintError::CustomHint(
        "tx.class_hash is None".to_string().into_boxed_str(),
    ))?;
    let component_hashes: &HashMap<Felt252, Vec<Felt252>> =
        exec_scopes.get_ref(vars::scopes::COMPONENT_HASHES)?;

    let class_component_hashes = component_hashes.get(&tx_class_hash).ok_or_else(|| {
        HintError::CustomHint(
            format!(
                "No component hashes found for class {}",
                tx_class_hash.to_hex_string()
            )
            .into_boxed_str(),
        )
    })?;
    ...
    let class_component_hashes: Vec<_> = class_component_hashes
        .iter()
    ...
    insert_value_from_var_name(
        vars::ids::CONTRACT_CLASS_COMPONENT_HASHES,
        arg_segment,
        vm,
        ids_data,
        ap_tracking,
