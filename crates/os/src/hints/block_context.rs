...
use crate::r#type::casm_contract_class::GenericCasmContractClass;
use crate::r#type::chain_id::chain_id_to_felt;
use crate::starknet::core::os::contract_class::compiled_class_hash_object::BytecodeSegmentStructureImpl;
...
pub fn load_class(
    ...
    let computed_hash =
        vm.get_integer((compiled_class_fact_addr + CompiledClassFact::hash_offset())?)?;
    let expected_hash = exec_scopes.get::<Felt252>(vars::scopes::COMPILED_CLASS_HASH)?;
    ...
    let compiled_class_ptr =
        get_ptr_from_var_name(vars::ids::COMPILED_CLASS, vm, ids_data, ap_tracking)?;
    let byte_code_ptr =
        vm.get_relocatable((compiled_class_ptr + CompiledClass::bytecode_ptr_offset())?)?;

    let mut hint_extension = HintExtension::new();

    let cairo_lang_class = class
        .to_cairo_lang_contract_class()
        .map_err(|e| custom_hint_error(e.to_string()))?;
...
pub fn block_timestamp(
    ...
    insert_value_into_ap(
        vm,
        Felt252::from(block_context.block_info().block_timestamp.0),
    )
}
...
pub fn deprecated_fee_token_address(
    ...
    let os_input = exec_scopes.get::<Rc<StarknetOsInput>>(vars::scopes::OS_INPUT)?;
    let deprecated_fee_token_address = *os_input
        .general_config
        .starknet_os_config
    ...
    insert_value_into_ap(
        vm,
        Felt252::from_bytes_be(&deprecated_fee_token_address.to_bytes_be()),
    )
}

pub const ELEMENTS_GE_10: &str =
...
pub fn fee_token_address(
    ...
    let os_input = exec_scopes.get::<Rc<StarknetOsInput>>(vars::scopes::OS_INPUT)?;
    let fee_token_address = *os_input
        .general_config
        .starknet_os_config
        .fee_token_address
        .0
    ...
    insert_value_into_ap(vm, Felt252::from_bytes_be(&fee_token_address.to_bytes_be()))
}
        ...
        Dictionary::SimpleDictionary(dict) => dict
            ...
            .ok_or_else(|| {
                HintError::CustomHint(
                    "State changes dictionary shouldn't be None"
                        .to_string()
                        .into_boxed_str(),
                )
            })?
...
pub fn load_class_facts(
    ...
    let os_input: Rc<StarknetOsInput> = exec_scopes
        .get::<Rc<StarknetOsInput>>(vars::scopes::OS_INPUT)?
        .clone();
    let compiled_class_facts_ptr = vm.add_memory_segment();
    insert_value_from_var_name(
        vars::ids::COMPILED_CLASS_FACTS,
    ...
    let compiled_class_facts: Box<dyn Any> =
        Box::new(os_input.compiled_classes.clone().into_iter());
    let compiled_class_visited_pcs: Box<dyn Any> =
        Box::new(os_input.compiled_class_visited_pcs.clone());
    exec_scopes.enter_scope(HashMap::from([
        (
            String::from(vars::scopes::COMPILED_CLASS_FACTS),
            compiled_class_facts,
        ),
        (
            String::from(vars::scopes::COMPILED_CLASS_VISITED_PCS),
...
pub fn load_class_inner(
    ...
    let class_iter = exec_scopes.get_mut_ref::<IntoIter<Felt252, GenericCasmContractClass>>(
        vars::ids::COMPILED_CLASS_FACTS,
    )?;

    let (compiled_class_hash, class) = class_iter.next().ok_or(HintError::CustomHint(
        "Compiled class iterator exhausted"
            .to_string()
            .into_boxed_str(),
    ))?;
...
pub fn sequencer_address(
    ...
    let block_context = exec_scopes.get_ref::<BlockContext>(vars::scopes::BLOCK_CONTEXT)?;
    let sequencer_addr = *block_context.block_info().sequencer_address.0.key();
    insert_value_into_ap(vm, Felt252::from_bytes_be(&sequencer_addr.to_bytes_be()))
}
...
pub fn write_use_kzg_da_to_mem(
    ...
    let use_kzg_da_felt = if use_kzg_da && !full_output {
        Felt252::ONE
    } else {
        Felt252::ZERO
    };

    vm.insert_value((vm.get_fp() + 18)?, use_kzg_da_felt)
